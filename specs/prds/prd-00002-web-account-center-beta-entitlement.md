...
|------|------|
| **状态** | 产品：`backlog` |
| **范围** | 官网 `/account` 账号中心、公测 `pro_beta` 权益授予与展示、Web BFF `GET /api/web/account`、`user_profiles` 业务表；**承接** [`prd-00001-phone-otp-auth.md`](prd-00001-phone-otp-auth.md) R2 |
...
| G6 | 公测开关 `BETA_ENROLLMENT_OPEN` 控制新用户授予；关闭后新用户 `free` |
| G7 | 老用户补授：公测仍开放且 `auth.users.created_at` 在公测期内 → 首次访问 account 补授 `pro_beta` |
...
| 质量（R1） | `pnpm test` 覆盖 `ensureProfile` 幂等与公测开关；契约文档与实现对齐 |
...
| **公测开关** | 环境变量 `BETA_ENROLLMENT_OPEN`（默认 `true`），控制新用户是否授予 `pro_beta` |
...
| # | 主题 | 已定 | 说明 |
|---|------|------|------|
| R1 | 公测注册即 `pro_beta` | **是** | 公测开关开启时，首次 OTP 验证成功或首次读取 account 时服务端授予 |
...
| R7 | 公测结束后新用户 | **是** | `BETA_ENROLLMENT_OPEN=false` 时新用户默认 `free`，历史 `pro_beta` 保留 |
| R8 | API 拆分 | **是** | 保留轻量 `GET /me`（Header 用），新增 `GET /api/web/account` 专供账号页 |
| R9 | 老用户追溯授予 | **是** | 公测仍开放且 `auth.users.created_at` 在公测期内 → 首次访问 account 补授 `pro_beta` |
| R10 | profile 创建双路径 | **是** | `verify` + GET account 均触发幂等 upsert（双保险） |
...
| **运维** | 通过 `BETA_ENROLLMENT_OPEN` 控制公测收官 |
...
**公测开关**：环境变量 `BETA_ENROLLMENT_OPEN=true`（默认 true），或 [`lib/site-config.ts`](../../lib/site-config.ts) 读取；BFF 授予逻辑读取。
...
**行为**：若 profile 不存在则幂等创建；公测开启且符合授予条件则 `plan = pro_beta`；老用户按 R9 补授。
...
| 故事 | 验收要点 |
|------|----------|
| 作为平台工程，我需要在 Supabase 持久化用户方案与权益 | migration 应用后存在 `public.user_profiles`、RLS 启用、anon 无法任意读写他人行 |
| 作为官网后端，我需要在 verify 与 account 读取时幂等 upsert 权益 | 公测开启 + 首次 OTP 成功 → `plan = pro_beta`、`entitlement_status = active`；重复调用不产生重复行 |
| 作为老用户，公测期内注册但无 profile，首次访问 account 应补授 | 公测开放 + `auth.users.created_at` 在公测期内 → 首次 GET account 授予 `pro_beta` |
...
| 作为工程，我想有授予与幂等单测 | 连续两次 `ensureProfile` 仅一条 profile；公测关闭时新用户为 `free` |
...
| Release | 交付物 | 可验收结果 |
|---------|--------|------------|
| **R0 MVP** | `user_profiles` migration、`lib/account/*`、`GET /api/web/account`、verify 扩展、`/account` 页、Header/定价联动 | 新用户登录后可见 `pro_beta`；未登录不可访问 account |
| **R1** | `web-auth-account.md`、OpenAPI 片段、Vitest、公测开关 env、老用户补授策略、503 错误态 UI、隐私复核 | `pnpm test` 通过；文档与实现对齐 |
...
| ID | 摘要 | Release |
|----|------|---------|
| FEAT-00001-01-DB | `user_profiles` 表与 RLS migration | R0 |
...
flowchart TD
  ...
  hasProfile -->|no| upsert[幂等 upsert]
  upsert --> betaCheck{公测开启且符合条件?}
...
| 存储 | R0 | 说明 |
|------|-----|------|
| `auth.users`（Supabase） | 是 | 已有；`created_at` 用于老用户补授判定 |
| `public.user_profiles` | 是 | 本 PRD 新建；方案与权益事实源 |
| 官网 HttpOnly Cookie | 是 | 沿用登录 PRD 会话 |
| `BETA_ENROLLMENT_OPEN` | R1 | 公测收官开关 |
...
**剧本 D — 公测收官**：`BETA_ENROLLMENT_OPEN=false` 后新用户 `free`；历史 `pro_beta` 不变。

**剧本 E — 老用户补登**：仅有 `auth.users`、公测期内 `created_at` → 首次 `/account` 补授 `pro_beta`。
...
| 5 | 公测期时间边界（老用户补授） | 以 `auth.users.created_at` 与公测结束日配置为准；R9 已拍板补授策略 |
...
| 日期 | 说明 |
|------|------|
| 2026-06-22 | 初稿：由 [`feat-00001-web-account-center-beta-entitlement`](../features/feat-00001-web-account-center-beta-entitlement.md) 升格；承接登录 PRD R2；老用户公测期内补授 `pro_beta` 已拍板 |
