...
**OpenAPI（开发环境）**：机器可读规范见 [`specs/openapi/web-auth.openapi.json`](../openapi/web-auth.openapi.json)；本地 `pnpm dev` 后访问 `http://localhost:3000/api/openapi` 或 `http://localhost:3000/api/openapi.json`。生产环境上述端点返回 404。
...
| 日期 | 说明 |
|------|------|
| 2026-06-22 | 新增 OpenAPI 3.0 JSON 规范（`specs/openapi/web-auth.openapi.json`），dev 专用 `/api/openapi.json` |
